...
function GameStateValidator({ children }) {
    ...
    useEffect(() => {
        ...
        if (location.pathname.startsWith('/game') && userTeamId) {
            ...
            // Check if user's squad has players (required after auction completes)
            // During preseason before auction, empty squads are legitimate
            const userSquad = squadLists?.[userTeamId];
            const hasUserSquad = userSquad && userSquad.length > 0;
            const requiresSquad = currentPhase && currentPhase !== 'preseason';

            // If we have a userTeamId but missing critical data, redirect to recovery
            // Data loss scenarios:
            // 1. teams data is empty (initializeTeams overwrote hydrated data)
            // 2. currentPhase is missing
            // 3. Past preseason but user squad is empty (squadLists got overwritten)
            if (!hasTeams || !hasGamePhase || (requiresSquad && !hasUserSquad)) {
    ...
    }, [location.pathname, userTeamId, teams, squadLists, currentPhase, navigate]);
